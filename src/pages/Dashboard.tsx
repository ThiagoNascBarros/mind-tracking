...
function Dashboard() {
  ...
  useEffect(() => {
    const carregarPontuacao = async () => {
      ...
      try {
        const response = await fetch(`http://34.200.62.154:3000/questionario/pontuacao/${user.id}`, {
          headers: {
    ...
    const gerarDica = async () => {
      try {
        ...
        const response = await fetch('http://34.200.62.154:3000/api/dica', {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}`
    ...
    const carregarHistoricoQuestionarios = async () => {
      ...
      try {
        const res = await fetch(`http://34.200.62.154:3000/questionario/historico/${user.id}`, {
          headers: {
            "Authorization": `Bearer ${token}`,
            "Content-Type": "application/json"
          }
